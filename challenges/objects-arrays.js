...
const universities = getUniversities(graduates);
...
function getContactInfo(obj) {
  let info = [];
for (i = 0; i < graduates.length; i++) {
  info.push(graduates[i].first_name + ' ' + graduates[i].email);
  }
  return info;
}
const contactInfo = getContactInfo(graduates);
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
function findUnis(obj) {
  let unis = [];
  for (i = 0; i < graduates.length; i++) {
    if (graduates[i].university.includes('Uni') === true) {
      unis.push(graduates[i]);
    }
  }
  return unis;
}
const uni = findUnis(graduates);
console.log(uni);
...
zooAnimals.map(function(obj) {
  lowerCase.push(`Name: ${obj.animal_name}`);
});
